...
<Tabs groupId="unraid-permissions" queryString>
  ...
  |---|---|
  | **Name** | `seerr` |
  | **Repository** | `ghcr.io/snapetech/seerrng:latest` |
  | **Icon URL** | `https://raw.githubusercontent.com/snapetech/seerrng/main/public/android-chrome-512x512.png` |
  ...
  **Variable (optional):**
  ...
  For persistent sign-in, configure [built-in HTTPS](/using-seerr/advanced/built-in-tls),
  an HTTPS reverse proxy, or explicitly enable `SEERR_ALLOW_HTTP_AUTH=true` for
  a trusted LAN.
