...
This page builds SeerrNG itself. To build the optional BookshelfNG
Readarr-compatible backend without Docker, use the [BookshelfNG source-build
guide](/using-seerr/bookshelf-source-build) instead.
:::
...
```bash
git clone https://github.com/snapetech/seerrng.git .
git checkout main
```
...
:::info
You can open the interface at `http://localhost:5055`. For authenticated
sessions, use [built-in HTTPS](/using-seerr/advanced/built-in-tls), an HTTPS
reverse proxy, or explicitly opt in to the trusted-LAN HTTP fallback.
:::
