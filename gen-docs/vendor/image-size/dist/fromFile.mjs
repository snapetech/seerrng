...
var HEIF = {
  ...
  calculate(input) {
    ...
    while (currentOffset < ipcoBox.offset + ipcoBox.size) {
      ...
      if (!ispeBox) break;
      if (ispeBox.size <= 0) {
        throw new TypeError("Invalid HEIF, ispe box has zero length");
      }
...
var ICNS = {
  ...
  calculate(input) {
    ...
    while (imageOffset < fileLength && imageOffset < inputLength) {
      ...
      if (imageHeader[1] <= 0) {
        throw new TypeError("Invalid ICNS, image entry has zero length");
      }
...
function extractPartialStreams(input) {
  ...
  while (offset < input.length) {
    ...
    if (!jxlpBox) break;
    if (jxlpBox.size <= 12) {
      throw new TypeError("Invalid JXL, jxlp box has invalid length");
    }
    partialStreams.push(
